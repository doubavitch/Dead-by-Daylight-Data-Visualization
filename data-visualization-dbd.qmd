...
pick_rate_plot <- ggplot(dead_by_daylight, aes(y = fct_infreq(Killer))) +
  geom_bar(fill = "#9C2A23") +
  geom_text(aes(label = stat(count)), stat = "count", hjust = 1, nudge_x = -.5, size = 2.7, colour = 'white') +
  guides(fill = 'none') +
...
killer_performance_plot <- ggplot(killer_perf, aes(x = kill_rate, y = fct_infreq(Killer, kill_rate))) +
  geom_bar(fill = "#9C2A23", stat = "identity", position = "dodge") +
  geom_text(aes(label = round(kill_rate, 3)), hjust = 0, nudge_x = -.4, size = 2.7, colour = 'white') +
  labs(title = "Killer performances", x = "Average of killed survivors", y = '') 

grid.arrange(pick_rate_plot, killer_performance_plot, ncol=2)

## highlights the killers we are interested in
```
