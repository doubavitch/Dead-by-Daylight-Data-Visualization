...
ggplot() +
  geom_bar(aes(x = dead_by_daylight$`# Finished generators`), fill = "#9C2A23") +  
...
```{r}
ggplot() +
  geom_bar(aes(y = fct_infreq(dead_by_daylight$Map)), fill = "#9C2A23") +
  guides(fill = 'none') +
...
hatch_frequency_killer |> 
  ...
  labs(title = "Hach escape rates per killer", x = "Frequency of Hatch Escapes", y = "Killer")
```
...
hatch_frequency_map |> 
  ggplot() +
  geom_bar(aes(x = Frequency, y = fct_infreq(Map, Frequency)), fill = "#9C2A23", stat = "identity", position = "dodge") +
  labs(title = "Hach escape rates per map", x = "Frequency of Hatch Escapes", y = "Map")
```
...
dead_by_daylight |> 
ggplot() +
geom_bar(aes(y = fct_infreq(Map) , fill = dead_by_daylight$`# Hatch escapes`), fill = "#9C2A23") +
#facet_wrap(~Killer) +
...
cor_data <- dead_by_daylight |> 
  group_by(Map, Killer) |> 
  summarise(Correlation = cor(`# Finished generators`, `# Escaped Survivors`))

ggplot(cor_data, aes(x = Map, y = Killer)) +
  geom_tile(aes(fill = Correlation)) +
  theme(axis.text.x=element_text(angle = 90, hjust = 0)) +
  scale_fill_continuous(na.value = '#6D6D6D') +
  scale_fill_gradient(low="#9C2A23", high="#2E63A1") +#2E63A1
  labs(x = "Map",
       y = "Killer",
