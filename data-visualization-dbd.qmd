...
#kills_distribution <- dead_by_daylight |> 
  ...
  #summarise(Matches = n()) |> 
  #pivot_wider(names_from = `# Escaped Survivors`, values_from = Matches)
#colnames(kills_distribution) <- c("Killer", "0 kill", "1 kill", "2 kills", "3 kills", "4 kills")

#kills_distribution_long <- kills_distribution |> 
  #pivot_longer(cols = `0 kill`:`4 kills`, names_to = "Kills", values_to = "Matches")
...
likert_plot <- HH::likert(Killer ~ ., kills_distrib_perc, main="Distributions of the kills per killer", 
                          ...
                          panel=function(...) { 
                            args <- list(...)
                            panel.text(args$x, args$y, pos=3, offset=0, labels = sprintf("%.1f%%", args$percentage))
                            panel.barchart(...)
              }
              )
...
```{r}
# count the most common comments
# Farm - Killer let us escape through the gates : blue
# Tunnel - Camp - Idiot : red
...
comments_bhvr <- dead_by_daylight |> 
  summarise(total_matches = n(),
            ...
            #DC_count = sum(str_detect(replace_na(Comments, ''), regex("DC", ignore_case = TRUE))),
            #DC_perc = (DC_count / total_matches) * 100,
            #kill_let_esc_count = sum(str_detect(replace_na(Comments, ''), regex("killer let us escape through the gates", ignore_case = TRUE))),
            #kill_let_esc_perc = (kill_let_esc_count / total_matches) * 100,
            #fair_count = sum(str_detect(replace_na(Comments, ''), regex("fair", ignore_case = TRUE))),
            #fair_perc = (fair_count / total_matches) * 100,
            #skilled_count = sum(str_detect(replace_na(Comments, ''), regex("skilled", ignore_case = TRUE))),
            #skilled_perc = (skilled_count / total_matches) * 100,
            #given_count = sum(str_detect(replace_na(Comments, ''), regex("given", ignore_case = TRUE))),
            #given_perc = (given_count / total_matches) * 100,
            #good_count = sum(str_detect(replace_na(Comments, ''), regex("good", ignore_case = TRUE))),
            #good_perc = (good_count / total_matches) * 100)
            
comments_bhvr

bm_summary <- tibble(Type_of_BM = c("BM", "idiot", "tunnel", "camp"),
                     Count = c(comments_bhvr$BM_count, comments_bhvr$idiot_count, comments_bhvr$tunnel_count, comments_bhvr$camp_count),
                     Percentage_total = c(100, comments_bhvr$idiot_perc_tot, comments_bhvr$tunnel_perc_tot, comments_bhvr$camp_perc_tot),
                     Percentage_BM = c(comments_bhvr$BM_perc, comments_bhvr$idiot_perc_bm, comments_bhvr$tunnel_perc_bm, comments_bhvr$camp_perc_bm))
...
# gather in one plot all the BM and total it up by the number of matches where we made a comment about any of those behaviours
